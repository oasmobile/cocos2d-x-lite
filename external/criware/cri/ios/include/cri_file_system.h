 ...
 * \par 説明:
 * 未使用バインダに与えられるIDです。バインドに失敗した時にも返ってくることがあります。<br>
 ...
 * バインダにバインドを行うと、バインドIDが作成されます。個々のバインドを識別するために使用します。<br>
 * - ファイルのバインドとアンバインド<br>
 ...
 * - バインダのプライオリティ<br>
 * バインダは、目的のファイルがどのバインドIDにあるのかを検索します。<br>
 ...
 * \ingroup FSLIB_BINDER
 * \par 説明：
 * バインダに対してバインドを行うと、 CriFsBindId (バインドID)が作成されます。<br>
...
typedef struct CriFsBinderInfoTag {
	...
	CriFsBindId bndrid;				/*JP< 他バインダを参照している場合の参照先バインドID<br>
 ...
 * \brief ロードリミッタ番号
 * \par 説明:
 * 　デフォルトの状態では、全てのローダハンドルがリミッタ０番（リミッタ制限なし）に割り当てられています。<br>
 * 　リミッタを利用する場合、制限を加えるローダ、グループローダ、バッチローダのハンドルにリミッタ１番を設定してください。<br>
 ...
 * （マルチスレッドモデル以外の場合、サーバ処理内でコピー処理が行われます。）<br>
 * <br>
 ...
 * <br>
 * ・CriFsLoader<br>
 * ::criFsLoader_Load 関数実行直後にローダのステータスが CRIFSLOADER_STATUS_COMPLETE に遷移します。<br>
 ...
 * \param[in]	limiter_size	ロードリミッタサイズ（byte/1サーバ周期）
 * \return	CriError			エラーコード
 * \par 説明:
 * １サーバ周期当たりの読み込みサイズの制限値を設定します。<br>
 * 共通のリミッタ番号を割り当てた全てのローダ、グループローダ、バッチローダの合計読み込みサイズが、ここで設定するサイズに制限されます。<br>
 * 制限に達したリミッタ番号のロード処理は一時保留され、次回のサーバ周期で自動的に再開します。<br>
 ...
 * \brief リード要求回数の制限
 * \ingroup FSLIB_CRIFS_EMB
 * \param[in] limit_num_read_request  １サーバ周期当たりのリード要求回数の上限
 * \return  CriError エラーコード
 * \par 説明：
 * １サーバ周期当たりのリード要求回数の上限を設定します。<br>
 ...
 * バインダを破棄します。<br>
 * \par 注意：
 * 破棄するバインダにバインドされているバインドIDも同時に破棄されます。<br>
 * 本関数で破棄できるのは、::criFsBinder_Create 関数により生成されたバインダハンドルのみです。<br>
 * ::criFsBinder_GetHandle 関数により CriFsBindId から取得されたバインダハンドルは破棄できません。<br>
 ...
 * 本関数の引数は、CPKビルドの際に出力されるCPKヘッダファイルのシンボル
 ...
 * 本関数は、バインダ(bndrhn)にCPKファイル(path)をバインドし、バインドID(bndrid)を返します。<br>
 ...
 * \brief ファイルのバインド
 * \ingroup FSLIB_BINDER
 * \param[in/out]	bndrhn	ファイルバインドをするバインダハンドル
 * \param[in]	srcbndrhn	バインド対象のファイルを検索するためのバインダハンドル
 * \param[in]	path		バインドするファイルのパス名
 ...
 * srcbndrhnのバインダからpathで指定されたファイルを検索し、bndrhnにバインドします。
 ...
 * \ingroup FSLIB_BINDER
 * \param[in/out]	bndrhn		ファイルセクションバインドをするバインダハンドル
 * \param[in]	srcbndrhn		バインド対象のファイルを検索するためのバインダハンドル
 * \param[in]	path			バインドするファイルのパス名
 ...
 * // 備考）アロケータが事前に登録済みであればワーク領域サイズの指定は不要。
 ...
 * 本関数は即時復帰関数です。<br>
 * 指定されたバインドIDを削除できない場合、CRIERR_NGを返します。<br>
 * アンバインド処理を進行させるには、サーバ処理 criFsExecuteMain 関数と、criFsBinder_GetStatus 関数の
 * 呼び出しが必要です。<br>
 *
 * \par アンバインド完了の検知：
 ...
 * ファイルがバインダに登録されていなければファイルI/Oにアクセスしてファイルの有無を調べる、
 * という実装になっています。<br>
 * （バインダに登録されていないファイルや、存在しないファイルに対して本関数を実行した場合、
 * ファイルI/Oへのアクセスが発生し、長時間処理がブロックされる可能性があります。）<br>
 * <br>
 * ファイルがバインダに登録されているかどうかをチェックしたい場合には、
 * 本関数の代わりに、 ::criFsBinder_Find 関数をご利用ください。<br>
 * \attention
 * 以下のケースでは、本関数内で長時間処理がブロックされる場合があります。
 * 	- バインダハンドルにNULLを指定した場合
 * 	- バインダに登録されていないファイルを指定した場合
 ...
 * bndrhn のバインダから目的のファイルを検索します。 <br>
 * バインド状態が CRIFSBINDER_STATUS_COMPLETE であるバインドIDが検索対象とします。<br>
 * バインダの使用は必須です。バインダハンドルにはNULLは指定できません。
 ...
 * プライオリティにより、バインダハンドル内における、バインドIDの検索順を制御できます。<br>
 ...
 * 格納しているCPKが選択されます。<br>
 * 特定のCPKファイルを直接指定したい場合、criiFsBinder_GetHandle関数により、そのCPKのバインドIDから
 * バインダハンドルを取得し、本関数の引数としてください。<br>
 * \par 注意：
 ...
 * \return		CriError	エラーコード
 * \par 説明：
 * 指定されたバインドIDのバインダ種別（CPK/ファイル/ディレクトリ等）やバインドしたファイル名、プライオリティ設定などの
 * 情報を取得します。<br>
 */
 ...
 * \param[out]	internal_loader_status	内部ローダステータス
 * \return		CriError				エラーコード
 * \par 説明：
 ...
 * \ingroup FSLIB_BINDER
 * \param[in]	bind_id				バインドID
 * \param[out]	internal_loader		内部ローダハンドル
 * \return		CriError			エラーコード
 * \par 説明：
 * バインド中のファイルを一旦クローズします。<br>
 ...
 * で返されるローダハンドルを使用してチェックします。<br>
 * （クローズ処理が完了したタイミングで、ローダハンドルのステータスが
 * ::CRIFSLOADER_STATUS_COMPLETE に遷移します。）<br>
 * \attention
 * 第 2 引数（ internal_loader ）で返されるローダハンドルは、
 ...
 * で返されるローダハンドルを使用してチェックします。<br>
 * （オープン処理が完了したタイミングで、ローダハンドルのステータスが
 * ::CRIFSLOADER_STATUS_COMPLETE に遷移します。）<br>
 ...
 * <br>
 * リードエラー等によりファイルの再オープンに失敗した場合、
 * 第 2 引数（ internal_loader_status ）で返されたローダハンドルのステータスが
 ...
 * ::criFsBinder_ReopenFileAsync 関数の実行で取得したローダハンドルのステータスが<br>
 ...
 * <br>
 * フィルタに指定できる文字の最大数は、現状 7 文字までです。<br>
 */
 ...
 * ロード処理自体が開始されない場合（ローダのステータスが CRIFSLOADER_STATUS_LOADING
 ...
 * <br>
 * ロード完了コールバック内でローダのステータス遷移を待つ処理を行わないでください。<br>
 * ローダのステータス更新は、ロード完了コールバックと同一のスレッド上で行われます。<br>
 * そのため、ロード完了コールバック内でステータス遷移を待つ処理を行うと、
 * デッドロックが発生し、処理が進まなくなります。<br>
 * <br>
 * コールバック関数はローダ内のメモリ領域に対して登録されます。<br>
 * そのため、ロード完了コールバック内でローダを破棄することはできません。<br>
 ...
 * 暗号化解除関数が登録されると、
 * ローダはロードした全てのファイルに対して暗号化解除関数を一度ずつコールバックします。<br>
 ...
 * CRIERR_NGを返す事でローダをエラー状態に遷移させる事ができ、
 ...
 * 本関数を実行することでI/Oインターフェースから返されたエラーコードを
 * 取得することが可能です。
 * \sa criFsLoader_GetStatus
 */
/*EN
 ...
 * 共通のリミッタ番号を割り当てた全てのローダ、グループローダ、バッチローダの合計読み込みサイズが制限されます。<br>
 * \attention
 * ゲーム機向けではロードリミッタ機能は非サポートです。この関数は呼び出さないでください。<br>
 * \sa CriFsLoadLimiterNo criFs_SetLoadLimiterSize criFs_SetLoadLimiterUnit criFsLoader_SetLoadLimiter criFsGroupLoader_SetLoadLimiter criFsBatchLoader_SetLoadLimiter
 */
 ...
 * \return		CriError		エラーコード
 * \par 説明：
 * バインドIDからグループローダを作成し、グループローダハンドルを返します。<br>
 * 本関数は完了復帰関数です。<br><br>
...
/*JP
 * \brief バインダハンドル対応のグループローダの作成
 * \ingroup FSLIB_GROUPLOADER
 * \param[in]     bndrhn        バインダハンドル(複数CPKファイルをバインドしているバインダ)
 * \param[in]     groupname     グループ名(NULL指定は不可)
 * \param[in]     attrname      アトリビュート名(NULL指定可能)
 * \param[out]    grouploaderhn グループローダハンドル
 * \return        CriError      エラーコード
 * \par 説明:
 * バインダハンドルからグループローダを作成し、グループローダハンドルを返します。<br>
 ...
 * 解放し、グループローダのステータスを ::CRIFSLOADER_STATUS_STOP にします。<br>
 ...
 * \param err エラーコード
 * \par 説明:
 * グループローダ内で使用するローダの単位読み込みサイズを設定します。
 * CriFsLoaderは、大きなサイズのリード要求を処理する際、それを複数の小さな単位のリード処理に分割して連続実行します。<br>
 * この関数を使用することで単位リード処理サイズを変更することが可能です。<br>
 * リード要求のキャンセルや、高プライオリティのリード処理の割り込み等は、単位リードサイズ境界でのみ処理されます。<br>
 * そのため、ユニットサイズを小さく設定すると、I/O処理のレスポンスが向上します。逆に、ユニットサイズを大きく設定すると、ファイル単位の読み込み速度が向上します。
...
/*JP
 * \brief サーバ処理当たりの準備ファイル数の制限
 * \ingroup FSLIB_GROUPLOADER
 * \param[in] grouploaderhn     グループローダハンドル
 * \param[in] nfile_per_server  １サーバ当たりの準備処理ファイル数、または、特別値
 *                              CRIFS_GROUPLOADER_NO_PREPARATION_LIMIT
 * \return  CriError エラーコード
 * \par 説明：
 * グループローダの準備処理の処理方法を設定します。<br>
 ...
 * この問題を回避する手段として、グループローダの準備処理をサーバ処理に分散させる機能を追加しました。<br>
 * 特別値以外の数値を指定した場合、準備処理は criFsGroupLoader_LoadBulk 関数からサーバ処理に移管され、
 * 1回のサーバ処理で準備処理を行うファイルの数は指定した数に制限されます。<br>
 * グループローダのサーバ処理は、criFsGroupLoader_GetStatus 関数内で実行されます。<br>
 * <br>
 * 具体的なコードの例は以下のとおりです。
 * \par 例:
 * \code
 ...
 *   // ハンドルbとcの準備処理はサーバ処理で500ファイルづつ行う。ハンドル毎に個別に処理量を制限する。
 ...
 * \ingroup FSLIB_GROUPLOADER_EMB
 * \param[in] group_all   グループローダハンドル(グループ全体)
 * \param[in] group_attr  グループローダハンドル(特定アトリビュート)
 * \param[out] offset     グループ全体のCriFsGroupFileInfo構造体の配列に対する、特定アトリビュートが開始するオフセット
 * \return  CriError   エラーコード
 * \par 説明：
 * 下記の条件でグループローダを利用する場合に、この関数を利用すると検索処理効率を改善することができます。<br>
 * \par 適用条件:
 ...
 * グループロードを実行するハンドルは、グループ名とアトリビュート名を指定して、グループローダハンドルを作成してください。<br>
 * さらに１つ、検索だけを目的とした、グループ全体のグループローダハンドルを作成します。この時はグループ名だけを指定し、アトリビュート名にはCRI_NULLを指定してください。<br>
 * CriFsGroupFileInfo構造体配列については、グループ全体の配列を連続メモリとして確保してください。<br>
 * このグループ全体の配列を切り分けて、個々の criFsGroupLoader_LoadBulk 関数の引数に指定します。<br>
 * 個々のグループローダの動作と、グループ全体に対する検索処理が整合するためには、グループ全体の配列を適切に切り分ける必要があります。<br>
