 ...
 * （エラーコールバックが発生します。）<br>
 * <br>
 * サーバ処理スレッドは、CRI Atomライブラリで使用しているものと同じです（共用しています）。<br>
 * すでにCRI AtomライブラリのAPIでサーバ処理スレッドの設定を変更している場合、<br>
...
/*JP
 * \brief インストーラスレッドのプライオリティ設定
 * \ingroup FSLIB_CRIFS_PC
 * \param[in]	prio	スレッドのプライオリティ
 * \return	CriError	エラーコード
 * \par 説明:
 * インストーラスレッドのプライオリティを設定します。<BR>
 * 引数 prio には Win32 API のスレッド優先レベル（ SetThreadPriority 関数の引数）を指定します。<BR>
 * アプリケーションのメインスレッドよりも高いプライオリティを指定してください。<BR>
...
/*JP
 * \brief インストーラスレッドのプライオリティ取得
 * \ingroup FSLIB_CRIFS_PC
 * \param[out]	prio	スレッドのプライオリティ
 * \return	CriError	エラーコード
 * \par 説明:
 * インストーラスレッドのプライオリティを取得します。<BR>
 * \attention:
 * 本関数は、ライブラリ初期化時にスレッドモデルをマルチスレッドモデル
 * （ ::CRIFS_THREAD_MODEL_MULTI ）に設定した場合にのみ効果を発揮します。<br>
 * 他のスレッドモデルを選択した場合、本関数はエラーを返します。<br>
...
/*JP
 * \brief サーバ処理スレッドのアフィニティマスク設定
 * \ingroup FSLIB_CRIFS_PC
 * \param[in]	mask	アフィニティマスク
 * \return	CriError	エラーコード
 * \par 説明:
 * サーバ処理（ライブラリの内部処理）を行うスレッドのアフィニティマスクを設定します。<BR>
...
/*JP
 * \brief インストーラスレッドのアフィニティマスク設定
 * \ingroup FSLIB_CRIFS_PC
 * \param[in]	mask	アフィニティマスク
 * \return	CriError	エラーコード
 * \par 説明:
 * インストーラスレッドのアフィニティマスクを設定します。<BR>
 * 引数 mask には Win32 API のスレッドアフィニティマスク（ SetThreadAffinityMask 関数の引数）を指定します。<BR>
 * デフォルト状態ではアフィニティマスクは設定されていません。<BR>
 * \attention:
 * 本関数は、ライブラリ初期化時にスレッドモデルをマルチスレッドモデル
...
/*JP
 * \brief インストーラスレッドのアフィニティマスク取得
 * \ingroup FSLIB_CRIFS_PC
 * \param[out]	mask	アフィニティマスク
 * \return	CriError	エラーコード
 * \par 説明:
 * インストーラスレッドのアフィニティマスクを取得します。<BR>
 * \attention:
 * 本関数は、ライブラリ初期化時にスレッドモデルをマルチスレッドモデル
 * （ ::CRIFS_THREAD_MODEL_MULTI ）に設定した場合にのみ効果を発揮します。<br>
 * 他のスレッドモデルを選択した場合、本関数はエラーを返します。<br>
